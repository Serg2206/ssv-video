...
Если вы работаете на **Windows** и хотите запустить генерацию видео локально —
используйте автономный установщик **`bootstrap.py`**. Он сам создаёт изолированное
окружение (venv), устанавливает все зависимости (`moviepy`, `pillow`, `gTTS`,
`imageio-ffmpeg`), **обходит необходимость в ImageMagick** (текст рисуется через
Pillow) и сразу генерирует тестовое видео для проверки.

```bat
:: 1. Установите Python 3.8+ (с галочкой "Add Python to PATH")
:: 2. Скачайте репозиторий и перейдите в его папку
cd C:\ssv-video
:: 3. Запустите установщик одной командой
python bootstrap.py
```

После завершения тестовое видео появится в `output\ssvproff_bootstrap_test.mp4`.

📖 **Полная пошаговая инструкция, проверка результата и решение типичных проблем —
в файле [WINDOWS_SETUP.md](WINDOWS_SETUP.md).**
